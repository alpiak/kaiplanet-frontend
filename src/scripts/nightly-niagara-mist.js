...
module.exports = function NightlyNiagaraMist(container) {
    +!~-(function(PIXI, window, document, undefined) {
        ...
        if (isCanvasSupported()) {
            ...
            var cw = c.width = Math.max(c.scrollWidth, c.offsetWidth, c.clientWidth, c.scrollWidth, c.offsetWidth);
            var ch = c.height = Math.max(c.scrollHeight, c.offsetHeight, c.clientHeight, c.scrollHeight, c.offsetHeight);
        ...
        function step() {
            if (waves.length < waveCount) {
                for (var i = 0; i < 10; i++) {
                    ...
                    wave.position.x = w * 0.8;
                    wave.position.y = h * 0.8;
                    wave.anchor.x = 0.5;
                    wave.anchor.y = 0.5;
            ...
            if (parts.length < partCount) {
                ...
                part.position.x = w * 0.8 + Math.random() * 380 - 190;
                part.position.y = h * 0.8 + 0;
                part.anchor.x = 0.5;
                part.anchor.y = 0.5;
